...
def single_pgd_attack(model, images, labels, pgd_type, pgd_loss, pgd_eps, 
    ...
    for _ in range(pgd_iters):
        ...
        images, momentum = pgd_update(images, ori_images, pgd_type, pgd_alpha, pgd_eps, n, momentum * mom_decay)
    return images
...
def comp_pgd_attack(comp_model, images, labels, pgd_type, pgd_loss,
    ...
    for _ in range(pgd_iters):
        ...
        images = images.clone().detach().requires_grad_(True)
        outputs, _, alphas = comp_model(images)

        if hasattr(pgd_loss, "bce_loss"):  # CompLoss and SimpleCompLoss have the attribute "bce_loss"
...
class PGDAdversary:
    def __init__(self, comp_model, randomized_attack_settings, pgd_settings):
        ...
        self.apgd_adversary = APGDAttack(self.comp_model, n_iter=self.pgd_iters_test, n_restarts=1,
    ...
    def pgd_attack_eval(self, images, labels, pgd_type, pgd_eps, pgd_alpha, pgd_iters,
        ...
        attack_loss = {'ce': self.ce_loss, 'dlr': self.dlr_loss_targeted,
                       'comp': self.comp_loss, 'comp_simple': self.comp_loss_simple}[loss_type]
        ...
        if loss_type == 'dlr':
            ...
            output, _, _ = self.comp_model(images)
            ...
            for tc in range(2, self.n_target_classes + 2):
                ...
                dlr_adv_imgs[tc - 2] = comp_pgd_attack(comp_model=self.comp_model, images=images, labels=labels,
        ...
        return comp_pgd_attack(comp_model=self.comp_model, images=images, labels=labels, pgd_type=pgd_type,
    ...
    def get_pgd_params(self, n):
        if self.randomized_attack_settings["randomize"]:
            if self.randomized_attack_settings["mode"] == "block":  # Randomize by batch
                eps_factor = uniform.rvs(loc=self.randomized_attack_settings["eps_factor_loc"],
                                         scale=self.randomized_attack_settings["eps_factor_scale"])  # Unif(.4, 1.1)
            elif self.randomized_attack_settings["mode"] == "sample":  # Randomize by instance
                ...
                raise ValueError("Unknown randomized attack mode.")
            cur_eps = self.pgd_eps * eps_factor
            # Number of iterations drawn from a Chi^2 distribution
        ...
        cur_iters = np.maximum(1, int(np.rint(iters_rv)))  # Make sure that at least one PGD iteration is scheduled
        cur_alpha = cur_eps / iters_rv * self.randomized_attack_settings["alpha_factor"]  # 1.8 for L_inf, 5 for L_2
    ...
    def randomized_pgd_attack(self, images, labels):
        ...
        cur_eps, cur_iters, cur_alpha, cur_mom_decay = self.get_pgd_params(images.shape[0])  # Get PGD hyperparameters
        old_w_ce = self.ce_loss.w
        self.ce_loss.w = torch.rand_like(labels.float()) * self.randomized_attack_settings["comp_loss_wmax"]
        ce_adv_imgs = comp_pgd_attack(comp_model=self.comp_model, images=images, labels=labels,
        ...
        for tc in range(2, self.n_target_classes + 2):
            ...
            self.dlr_loss_targeted.w = torch.rand_like(labels.float()) * self.randomized_attack_settings[
                "comp_loss_wmax"]  # Randomized comp loss
            cur_eps, cur_iters, cur_alpha, cur_mom_decay = self.get_pgd_params(images.shape[0])  # Get PGD parameters
            dlr_adv_imgs[tc - 2] = comp_pgd_attack(comp_model=self.comp_model, images=images, labels=labels,
    ...
    def randomized_apgd_attack(self, images, labels):
        ...
        self.apgd_adversary.w = torch.rand_like(labels.float()) * self.randomized_attack_settings["comp_loss_wmax"]
        self.apgd_adversary.loss = 'ce-randomized'
        self.apgd_adversary.eps, self.apgd_adversary.n_iter, _, _ = self.get_pgd_params(images.shape[0])
        self.apgd_adversary.init_hyperparam(x=images)
        ce_adv_imgs = self.apgd_adversary.attack_single_run(x=images, y=labels)[-1].detach().cpu()
        # print("Done with CE.")
        ...
        for tc in range(2, self.n_target_classes + 2):
            self.apgd_adversary = APGDAttack(self.comp_model, n_iter=self.pgd_iters_test, n_restarts=1, rho=.75,
                                             norm=self.type_convert[self.pgd_type], eps=self.pgd_eps, seed=0, 
                                             loss='dlr-randomized', eot_iter=1, comp=True, device=self.device)
            self.apgd_adversary.y_target = sorted_output[:, -tc]
            self.apgd_adversary.w = torch.rand_like(labels.float()) * self.randomized_attack_settings[
                "comp_loss_wmax"]  # Randomized comp loss
            self.apgd_adversary.eps, self.apgd_adversary.n_iter, _, _ = self.get_pgd_params(images.shape[0])
            ...
            self.apgd_adversary.init_hyperparam(x=images)
            dlr_adv_imgs[tc - 2] = self.apgd_adversary.attack_single_run(x=images, y=labels)[-1].detach().cpu()
