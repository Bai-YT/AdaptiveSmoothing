...
# @title Define ResNet
class PreActBlock(nn.Module):  # For ResNet
    expansion = 1
...
class BasicBlock(nn.Module):  # For ResNet
    expansion = 1
...
class WRNBasicBlock(nn.Module):  # For WRN
...
class NetworkBlock(nn.Module):  # For WRN
...
class Bottleneck(nn.Module):  # For ResNet
    expansion = 4
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, normalize=False):
        super().__init__()
        self.in_planes = 64
...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, drop_rate=0.0):
        super().__init__()
        ...
        # 1st sub-block
        self.sub_block1 = NetworkBlock(n, nChannels[0], nChannels[1], block, 1, drop_rate)
        # 2nd block
    ...
    def forward(self, x):
        feats1 = self.conv1(x)
        feats1 = self.block1(feats1)
        feats2 = self.block2(feats1)
        out = self.block3(feats2)
        out = self.relu(self.bn1(out))
        out = F.avg_pool2d(out, 8)
        out = out.view(-1, self.nChannels)
        out = self.fc(out)
        return out, (feats1, feats2)
...
def ResNet34(num_classes=10, normalize=False):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, normalize=normalize)


def ResNet50(num_classes=10, normalize=False):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, normalize=normalize)


def ResNet101(num_classes=10, normalize=False):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes=num_classes, normalize=normalize)


def ResNet152(num_classes=10, normalize=False):
    return ResNet(Bottleneck, [3, 8, 36, 3], num_classes=num_classes, normalize=normalize)
