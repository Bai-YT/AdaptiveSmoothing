    ...
    def _make_layer(self, in_planes, out_planes, num_blocks, stride,
        ...
        for i, stride in enumerate([stride] + [1] * (num_blocks - 1)):
            layers.append(_PreActBlock(i == 0 and in_planes or out_planes,
                                 out_planes, stride, activation_fn))
    ...
    def forward(self, x):
        ...
        if x.is_cuda:
            if self.mean_cuda is None:
                self.mean_cuda = self.mean.cuda()
                self.std_cuda = self.std.cuda()
            feat1 = (x - self.mean_cuda) / self.std_cuda
        else:
            feat1 = (x - self.mean) / self.std
        feat1 = self.conv_2d(feat1)
        feat1 = self.layer_0(feat1)
        feat2 = self.layer_1(feat1)
        out = self.layer_2(feat2)
