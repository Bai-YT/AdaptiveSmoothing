...
class APGDAttack():
    ...
    :param eot_iter:      iterations for Expectation over Transformation
    :param rho:           parameter for decreasing the step size
    :param comp:          indicates whether "predict" is a composite model
    :param w:             weight of the randomized loss functions
    """
    ...
    def get_gradient(self, criterion_indiv, x_adv, y, grad):
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if torch.is_tensor(self.eps) and len(self.eps.shape) > 1:
            assert self.eps.shape[0] == x.shape[0]
        if x_init is None:  # Random Init
            if self.norm == 'Linf':
                t = 2 * torch.rand_like(x, device=self.device).detach() - 1
                x_adv = x + self.eps * torch.ones_like(x).detach() * self.normalize(t)
            elif self.norm == 'L2':
                t = torch.randn_like(x, device=self.device).detach()
                x_adv = x + self.eps * torch.ones_like(x).detach() * self.normalize(t)
            elif self.norm == 'L1':
                t = torch.randn_like(x, device=self.device).detach()
        ...
        for _ in range(self.eot_iter):
            logits, loss_indiv, grad = self.get_gradient(criterion_indiv, x_adv, y, grad)
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if torch.is_tensor(self.eps) and len(self.eps.shape) > 1:  # Check whether eps is tensor
                        ...
                        if self.y_target is not None:
                            self.y_target = self.y_target[ind_to_fool]
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, y=None, x_init=None):
        ...
        :param y:           clean labels, if None we use the predicted labels
        """
        assert self.loss in ['dlr-targeted', 'dlr-randomized']  # 'ce-targeted'
        if not y is None and len(y.shape) == 0:
            x.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x)
