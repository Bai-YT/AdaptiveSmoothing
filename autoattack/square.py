...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        if not self.targeted:
            if self.loss == 'ce':
                return (y_corr - y_others).float(), (-1. * xent).float()
            elif self.loss == 'margin':
                return (y_corr - y_others).float(), (y_corr - y_others).float()
        else:
            return (y_others - y_corr).float(), xent.float()
    ...
    def attack_single_run(self, x, y):
        ...
        if self.norm == 'Linf':
            ...
            for _ in range(h // s):
                ...
                for _ in range(w // s):
                    delta_init[:, :, vh:vh + s, vw:vw + s] += self.eta(
                        s).view(1, 1, s, s) * self.random_choice([x.shape[0], c, 1, 1])
                    vw += s
                vh += s
