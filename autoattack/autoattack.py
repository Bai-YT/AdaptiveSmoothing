...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True, attacks_to_run=[], 
                 version='standard', is_tf_model=False, device='cuda', log_path=None):
        self.model = model
        self.norm = norm
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False, perform_checks=False):
        ...
        for batch_idx in range(n_batches):
            ...
            for attack in self.attacks_to_run:
                ...
                if num_robust == 0:  # All images are successfully attacked
                    break
                ...
                for batch_idx in range(n_batches):
                    ...
                    elif attack == 'apgd-ce-rand':  # apgd on cross-entropy loss
                        self.apgd.loss = 'ce-randomized'
                        self.apgd.w = COMP_LOSS_W
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'apgd-dlr':  # apgd on dlr loss
                        self.apgd.loss = 'dlr'
                        self.apgd.seed = self.get_seed()
                        adv_curr = self.apgd.perturb(x, y)  # cheap=True
                    elif attack == 'fab':  # fab
